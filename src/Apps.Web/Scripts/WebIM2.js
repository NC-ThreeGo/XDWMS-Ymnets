...
KindEditor.ready(function (K) {
    ...
    if (parent != null) {
        if (parent[2] != "")
            editor1.html(parent[2]);
        if (parent[0] != "" && parent[1] != "")
            $("#WaitMemberList").html("<span id='" + parent[0] + "'>[" + parent[1] + "]</span>")
    }
});
...
$(function () {
    ...
    $.post("../../Mis_Message/GetMemberTree", { "id": "0" }, function (data) {
        o.data = data;
    ...
    //初始化最近联系人
    //初始化常用语言
    $.post("../../Mis_Message/GetCommonTalk", function (data) { $("#CommonTalkList").html(data); }, "json");
...
function refresh_recentContact() {
    //定时更新
    $.post("../../Mis_Message/GetRecentContact", function (data) {
        //发到前台
        $("#LatertDiv").html(data);
    }, "json");
    window.setTimeout(refresh_recentContact, MIS0003);
}
...
$(function () {
    ...
    $("#SelTxt").keyup(function () {
        ...
        $.post("../../Mis_Message/GetSelMemberList", { whereStr: $("#SelTxt").val() }, function (data) {
            if (data != "") {
    ...
    $("#SendMessage").click(function () {
        ...
        //添加到数据库
        $.post("../../Mis_Message/SendMessageGetId",
                         { "mes": editor1.html()
                         , "receiver": receiver
...
function BindMemberListWin() {
    ...
    $(".webim-info-tit-name .webim-info-tit-thename").click(function () {
        if ($(this).parent().attr("class") == "current") {//选中自己，去掉自己样式，准备发送给列表的信息
            ...
            $.post("../../Mis_Message/GetChatContent"
            ...
            , function (data) {
                ...
                //读过了
                if (data != "") {
                    $.post("../../Mis_Message/SetMessageHasReadByReceiver", { memberId: $(".webim-info-tit-name .current span").attr("id") }, function (data) { }, "json");
                } else {
                    $.post("../../Mis_Message/GetChatContentIsReaded"
                    , { memberId: $(".webim-info-tit-name .current span").attr("id") }
                    , function (data) {
                        if (data != "") {
                            //发到前台
