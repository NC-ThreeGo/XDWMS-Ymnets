...
namespace Apps.Web.Areas.WMS.Controllers
{
    public class InventoryDController : BaseController
    {
        ...
        {
         ViewBag.Inventory = new SelectList(m_InventoryBLL.GetList(ref setNoPagerAscById, ""), "Id", "Name");
            return View();
        }

        [HttpPost]
        [SupportFilter]
        public JsonResult Create(WMS_Inventory_DModel model)
        {
            model.Id = 0;
            ...
            if (model != null && ModelState.IsValid)
            {
                ...
                {
                    LogHandler.WriteServiceLog(GetUserId(), "Id" + model.Id + ",HeadId" + model.HeadId, "成功", "创建", "WMS_Inventory_D");
                    return Json(JsonHandler.CreateMessage(1, Resource.InsertSucceed));
                }
                else
                {
                    string ErrorCol = errors.Error;
                    LogHandler.WriteServiceLog(GetUserId(), "Id" + model.Id + ",HeadId" + model.HeadId + "," + ErrorCol, "失败", "创建", "WMS_Inventory_D");
