...
{
    public class InOutCategoryController : BaseController
    {
        [Dependency]
        public ISpl_InOutCategoryBLL m_BLL { get; set; }
       ...
       [SupportFilter(ActionName="Index")]
        ...
        {
            List<Spl_InOutCategoryModel> list = m_BLL.GetList(ref pager, queryStr);
            GridRows<Spl_InOutCategoryModel> grs = new GridRows<Spl_InOutCategoryModel>();
            grs.rows = list;
        ...
        public JsonResult Delete(string id)
        {
            if (!string.IsNullOrWhiteSpace(id))
            {
                ...
                {
                    LogHandler.WriteServiceLog(GetUserId(), "Id:" + id, "成功", "删除", "Spl_InOutCategory");
                    return Json(JsonHandler.CreateMessage(1, Resource.DeleteSucceed));
                }
                else
                {
                    string ErrorCol = errors.Error;
                    LogHandler.WriteServiceLog(GetUserId(), "Id" + id + "," + ErrorCol, "失败", "删除", "Spl_InOutCategory");
